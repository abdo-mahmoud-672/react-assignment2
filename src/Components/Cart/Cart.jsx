...
export default function Cart() {
 ...
 useEffect(()=>{
  ...
  return <>
      ...
      <h2 className='font-semibold text-3xl mb-8'>Cart Shop</h2>
      {loading? <Loading/> : <div>
  ...
  <table className="w-full text-sm text-left mb-2 rtl:text-right text-gray-500 dark:text-gray-400">
    ...
    <tbody>
    {cart?.data.products.map((product , index)=>   <tr key={`${product.product.id}-${index}`} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
        <td className="p-4">
    ...
    <Link to={'/checkout'} className='bg-main text-center text-white p-2  rounded-md '>Check out</Link>
  </table>
